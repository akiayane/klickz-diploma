...
export class ShellComponent implements OnInit {
  ...
  links = ['profile/account', 'links', 'createlink'];
  titles = ['Профиль', 'Ссылки', 'Создать'];
  activeLink;
  ...
  ngOnInit(): void {
    //bruh();
    //console.log(this.router.url.slice(11));
    for (let i=0;i<this.links.length;i++){
      //console.log(this.links[i]);
      if (this.router.url.slice(11) == this.links[i]){
